...
const automaticReply = async (message, user, conversationId = null) => {
    ...
    if (/(paiement|payer|stripe|carte|mobile money|mpesa|airtel money|orange money)/.test(text)) return "Pour payer, ouvrez Mes achats puis choisissez une facture avec un reste à payer. Le bouton Payer en ligne permet un paiement complet ou partiel par carte. Si aucune facture n'apparait, votre commande doit d'abord etre validee et transformee en facture par l'equipe.";
    ...
    const aiReply = await generateBusinessReply({
        ...
        context: {
            entreprise: {
                ...
                ville: 'Goma',
                espace_client: ['Commandes', 'Mes achats', 'Reclamations', 'Assistance', 'Paiement en ligne si active']
            },
            catalogue,
